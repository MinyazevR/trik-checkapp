...
void Checker::reviewTasks(const QFileInfoList &qrsInfos,
                          ...
                          const QHash<QString, QVariant> &options)
{
    ...
    connect(&dialog, &QProgressDialog::canceled, &watcher, &QFutureWatcher<QHash<QString, QList<TaskReport>>>::cancel);
    connect(&watcher,
            &QFutureWatcher<QHash<QString, QList<TaskReport>>>::progressRangeChanged,
            &dialog,
            &QProgressDialog::setRange);
    connect(&watcher,
            &QFutureWatcher<QHash<QString, QList<TaskReport>>>::progressValueChanged,
            &dialog,
            &QProgressDialog::setValue);
    connect(&watcher, &QFutureWatcher<QHash<QString, QList<TaskReport>>>::finished, this, [this, &dialog, &watcher]() {
...
Checker::task_results_t Checker::checkTask(const Checker::Task *t)
{
    ...
    for (auto &&f : t->fieldsInfos) {
        ...
        report.message = executeProcess(execDir + QDir::separator() + "patcher" + ext,
        ...
        auto message = executeProcess(execDir + QDir::separator() + "2D-model" + ext,
